...
| `dressblock` | Dress block (连衣裙服装的基本型) | 英国经典服装纸样设计基础篇 (Bray) | 157–162 | **merged** | ⭐ **Now unblocked** — Ch.12 joins the bodice and skirt blocks, both shipped. Ch.11 §7/§8 (pp.155–156) give the side-seam and waist-dart matching rules it depends on |
...
| 3 | 衣片原型的使用 | 36–50 | ◐ partial | **option** | Theory digested (`bray-theory-notes.md` §3, all 12 sections). **§7 增大围度 back-check DONE** (merge `893e740fb02`) — it caps side-seam enlargement at 0.5–1cm fitted / 2–2.5cm loose, escalating to the shoulder route at 3–5cm, and checking the shipped options against it found **three defects in merged code that passed all 230 tests**: `armholeBridgeBonus` added 9.25cm of bust girth while its log claimed it spent ease (deleted; remedy ② is now guidance), the front armhole could invert and cross its own side seam on 5 of 20 adult models (invariant added), and the solver clamped on real adults while only warning, so CI stayed green over a sleeve that would not set in (now errors on adults). **Still owed:** 腋下省 p.41 and 中心肩缝 p.47 as options — Bray gives full numbers for both. References 尖点式育克 without giving the method; Ch.5 has it |
