...
The `mirror` macro allows you to mirror points, paths and/or snippets around a mirror line.

It is provided by the [mirror plugin](/reference/plugins/mirror/), which is
part of [core-plugins](/reference/plugins/core) (so it is available by default).

Note: mirroring a path will reverse its direction. As paths in FreeSewing should be oriented counter clockwise, this macro can optionally reverse the direction of mirrored paths back to the correct orientation.
...
;({ Point, points, Path, paths, macro, part }) => {
  ...
  paths.example = new Path().move(points.from).curve(points.cp1, points.cp2, points.to)
...
|   `snippets` |            | `array`    | An array of snippetnames, the names of Snippets in the `snippets` array to mirror                                                                                                        |
...
|    `reverse` | `false`    | `bool`     | Whether to reverse mirrored paths                                                                                                                                                        |
