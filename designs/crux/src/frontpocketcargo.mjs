...
export const frontPocketCargo = {
  name: 'crux.frontpocketcargo',
  after: [frontPocket],
  options: {},
  draft: ({ options, Point, points, Path, paths, sa, store, measurements, macro, part }) => {
    if (!options.frontPocketCargo) {
      return part.hide()
    }
    const cargoLength = store.get('frontPocketSeamCargoLength')
    const cargoWidth = measurements.waistToFloor * options.frontPocketCargoWidth
