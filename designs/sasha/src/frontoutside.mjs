...
import { frontInside as sashaFrontInside } from './frontinside.mjs'
...
export const frontOutside = {
  ...
  draft: ({
    ...
    if (options.withPocket) {
      ...
      // pocket should not start above waist height (pocket width would not be calculated correctly)
      // TODO: find a good way to extend the pocket opening above the waist and remove the restriction
      const pocketHeight = Math.min(
        options.pocketOpeningHeight * measurements.waistToHips,
        waistToWrist
      )
      ...
      log.info(`lowest point of pocket opening is ${units(waistToWrist)} below waist`)
