...
export const frontpoints = {
  ...
  draft: ({ options, measurements, Point, Path, points, paths, store, log, part }) => {
    const halfInch = measurements.waistToKnee / 48

    const inseam = measurements.inseam - (measurements.waistToFloor - measurements.waistToKnee)
    const seat = measurements.seat * options.pctSeatAdjustment
    const waist =
    ...
    const aCPu = points.pA.dist(points.pO) * options.pctAtoO
    const aCPj = points.pA.dist(points.pC) * options.pctAtoC
